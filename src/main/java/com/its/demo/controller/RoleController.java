...
//        response.setHeader("Content-disposition", "attachment;filename=" + fileName + ".xlsx");
        EasyExcel.write(response.getOutputStream(), RoleVO.class).sheet("部门").doWrite(listRoleVO);
    }
    ...
    public void insertRole(@RequestBody RoleDO role) {
    ...
    @ApiOperation(value = "检索角色", notes = "检索角色")
    @ApiImplicitParams({
            @ApiImplicitParam(name = "keyword", value = "关键字", dataType = "String", required = true, paramType = "path"),
            @ApiImplicitParam(name = "authorization", value = "令牌", required = true, paramType = "header")
    })
    public List<RoleDO> listRolesByKeyword(@PathVariable String keyword, @RequestHeader String authorization){
        return roleService.ListRolesByKeyword(keyword);
    }
}
